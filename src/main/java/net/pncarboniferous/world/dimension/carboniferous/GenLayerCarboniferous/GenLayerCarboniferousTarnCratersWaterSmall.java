...
public class GenLayerCarboniferousTarnCratersWaterSmall extends GenLayer
{
    ...
    public GenLayerCarboniferousTarnCratersWaterSmall(long seed, GenLayer genLayer)
    {
        super(seed);
    ...
    public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight)
    {
        ...
        for (int i = 0; i < areaHeight; ++i)
        {
            for (int j = 0; j < areaWidth; ++j)
            {
                ...
                if (k == CARBONIFEROUS_CRATER_ID && nextInt(2) == 0)
                {
