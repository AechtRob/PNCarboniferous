...
public class GenLayerCarboniferousIceEdgeSea extends GenLayer
{
    ...
    public GenLayerCarboniferousIceEdgeSea(long seed, GenLayer genLayer)
    {
        super(seed);
        ...
        for (int i1 = 0; i1 < p_151626_4_; ++i1)
        {
            for (int j1 = 0; j1 < p_151626_3_; ++j1)
            {
                ...
                if (k1 == CARBONIFEROUS_ICE_ID)
                {
                    ...
                    boolean flag = (
                            (l1 != CARBONIFEROUS_ICE_ID && l1 != CARBONIFEROUS_OCEAN_ID)
                            || (i2 != CARBONIFEROUS_ICE_ID && i2 != CARBONIFEROUS_OCEAN_ID)
                            || (j2 != CARBONIFEROUS_ICE_ID && j2 != CARBONIFEROUS_OCEAN_ID)
                            || (k2 != CARBONIFEROUS_ICE_ID && k2 != CARBONIFEROUS_OCEAN_ID)
                    );
                    if (flag)
                    {
