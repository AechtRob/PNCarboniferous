...
public class GenLayerCarboniferousHillsCentre extends GenLayer
{
    ...
    private final int HillsBiomes[] = new int[] {
        ...
        CARBONIFEROUS_HILLS_EDGE_ID,
        CARBONIFEROUS_HILLS_EDGE_ID,
        CARBONIFEROUS_HILLS_CENTRE_ID
    };

    private final int CentreBiomes[] = new int[] {
    ...
    public GenLayerCarboniferousHillsCentre(long seed, GenLayer genLayer)
    {
        super(seed);
        ...
        for (int i1 = 0; i1 < p_151626_4_; ++i1)
        {
            for (int j1 = 0; j1 < p_151626_3_; ++j1)
            {
                ...
                if (k1 == CARBONIFEROUS_HILLS_ID)
                {
                    ...
                    boolean flag = (
                        (l1 == CARBONIFEROUS_HILLS_ID)
                        && (i2 == CARBONIFEROUS_HILLS_ID)
                        && (j2 == CARBONIFEROUS_HILLS_ID)
                        && (k2 == CARBONIFEROUS_HILLS_ID)
                    );
                    if (flag)
                    {
                        k1 = CentreBiomes[nextInt(CentreBiomes.length)];
                    }
                }
                else if (k1 == CARBONIFEROUS_HILLS_EDGE_ID) {
                    if (nextInt(2) == 0) {
