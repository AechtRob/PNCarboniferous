...
import net.pncarboniferous.world.biome.carboniferous.BiomeCarboniferousMarsh;

public class GenLayerDiversifyCarboniferousBurntMarsh extends GenLayer {
    ...
    public GenLayerDiversifyCarboniferousBurntMarsh(long seed, GenLayer genlayer) {
        super(seed);
    ...
    private int[] diversify(int x, int z, int width, int height) {
        ...
        for (int zOut = 0; zOut < height; zOut++) {
            for (int xOut = 0; xOut < width; xOut++) {
                ...
                if (nextInt(2) == 0) {
                    if (Biome.getBiome(center) == BiomeCarboniferousMarsh.biome)
